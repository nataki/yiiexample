...
class StaticPageTest extends WebTestCase {
    public function testBrowseStaticPages() {
        ...
        foreach($staticPages as $staticPage) {
            $this->open($staticPage->action);
            $this->assertTextPresent($staticPage->title, 'Could not find title of the static page "'.$staticPage->action.'"!');
            ...
            $this->assertContains($staticPageNormalizedContent, $pageMainNormalizedContentHtml, 'Could not find content of the static page "'.$staticPage->action.'"!');
        }                
    }    
}
