<?php
/**
 * QsActiveRecordBehaviorFile class file.
 *
 ...
 * @package qs.db.ar
 */
class QsActiveRecordBehaviorFile extends CBehavior {
	/**
	 ...
	 */
	protected $_fileExtensionAttributeName = 'file_extension';
	/**
	 ...
	 */
	protected $_fileVersionAttributeName = 'file_version';
	/**
	...
	protected function initFileStorageBucketName() {
		$fileStorageBucketName = get_class($this->getOwner()).ucfirst($this->getFilePropertyName());
		$this->_fileStorageBucketName = $fileStorageBucketName;
		return true;
	}
	...
	protected function getSubDirPlaceholderValue($matches) {
		...
		switch ($placeholderName) {
			...
			case '__model__': {
				$owner = $this->getOwner();
				$placeholderValue = get_class($owner);
				break;
			}
			case '__file__': {
